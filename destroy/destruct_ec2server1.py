...
def destroy_ec2_server1(env_config, environment):
    try:
        ...
        for res in response['Reservations']:
            for inst in res['Instances']:
                ...
                print(f"🔍 Found Server1: {instance_name} ({inst['InstanceId']})")
        
        if not instance_ids:
            print(f"✅ No Server1 instance found: {instance_name}")
            return True
            
        # Terminate instance
        ec2.terminate_instances(InstanceIds=instance_ids)
        print("🗑️ Terminating Server1 instance...")
        
        # Wait for termination
        print("⏳ Waiting for instance to terminate...", end='', flush=True)
        waiter = ec2.get_waiter('instance_terminated')
        waiter.wait(InstanceIds=instance_ids)
        print("\n✅ Server1 instance terminated")
        return True
